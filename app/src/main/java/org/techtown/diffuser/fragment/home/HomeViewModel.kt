...
class HomeViewModel @Inject constructor(
    ...
    init {
        val defaultList = listOf(   //fetch 하여 데이터를 넣기 전에  뷰를 미리 구성하여 세팅.  fetch 전 로딩뷰를 보여주기 위함. 로딩뷰 default 로 보여주고 있다가 Fetch 이후 사라지게 .
            TitleModel(
                "인기영화",
                TheMore.THEMORE_POPULAR,
                HomeAdapter.VIEW_TYPE_TITLE,  //뷰타입 뷰홀더 생성과 온바인드시에 활용.
                HomeFragment.RECYCLERVIEW_ID_TITME   //diffUtil 에서 Item 비교시 활용. 이후 Movie id를 다시 넣어주어 고유값을 가지게 될 예정.
            ),
            ...
            WrappingModel(
                true, null,
                HomeAdapter.VIEW_TYPE_UPCOMMING, id = HomeFragment.RECYCLERVIEW_ID_COMMING  // 지금단계에서는 VIEW_TYPE_UPCOMMING 을 쓰나 VIEW_TYPE_NOW_MOVIE 를 쓰나 크게 상관 없음. 어짜피 같은 레이아웃 사용.
            )
        )
        ...
        repository
            ...
            .onEach { result ->  // 위의 이벤트 발생부분에 대한 처리부분.  이벤트 발생결과는 callApi 의 결과이므로 Resource seal 클래스타입으로 리턴받음.
                when (result) {
                    ...
                    is Resource.Success -> {
                        val response = result.model  //Resource<out T> 이기 때문에  model = BaseResponse가 됨  이경우 PopularMoviesResponse .
                        val list = response.results.map { //response의 results 리스트 결과값을 map으로  필요한 정보만 뽑아서 Movie 클래스에 저장 --> List<Moive> 가 됨.
                            Movie(
                                title = it.title,
                                rank = it.releaseDate,
                        ...
                        _items.value = _items.value!!.mapIndexed { index, itemModel -> // 라이브 데이터 업데이트.   이경우 기존 defaultList 의  index 1 에다가 map 을통해 값을 변경해서 넣고 hash코드 변경
